...
export async function verifyCloudBase(env, accessToken, fetcher = fetch) {
  ...
  if (!active || !subjectValid || !phoneValid || profile?.is_anonymous === true) {
    // Only structural diagnostics: never log the profile, subject, phone or token.
    const shape = {
      status: active ? 'active' : profile?.status == null ? 'missing' : 'non-active',
    ...
    const reason = !active ? 'PROFILE_STATUS' : !subjectValid ? 'PROFILE_SUBJECT' : !phoneValid ? 'PROFILE_PHONE' : 'PROFILE_ANONYMOUS';
