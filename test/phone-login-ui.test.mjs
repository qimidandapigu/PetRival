...
function fixture({ conflict = false } = {}) {
  ...
  class Element {
    ...
    append() {} setAttribute() {} querySelector(selector) { return get(selector.slice(1)); }
  ...
  let button, dialog;
  const document = { querySelector: () => get('host'), body: new Element(), createElement(tag) { const e = new Element(); if (tag === 'button') button = e; else dialog = e; return e; } };
  ...
  return { get, button, dialog, calls, sent, checked, reloads: () => reloads };
}
