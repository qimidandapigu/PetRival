...
export function validateStage(stage, state) {
  ...
  const cell = p => p && Number.isInteger(p.x) && Number.isInteger(p.y) && p.x >= 0 && p.x < w && p.y >= 0 && p.y < w ? p.y * w + p.x : -1;
...
export function previewCandidates(state, proposals, visits = new Map()) {
  ...
  return proposals.slice(0, 3).map((proposal, index) => {
    ...
    let current = state;
    const labeledBoxes = Object.fromEntries(state.boxes.map((b,i) => ['ABCD'[i], b]));
    ...
    for (const instruction of ids) {
      const id = typeof instruction === 'string' ? instruction :
        instruction && ['A', 'B', 'C', 'D'].includes(instruction.box) && ['U', 'D', 'L', 'R'].includes(instruction.direction)
      ...
      for (const label of ['A', 'B', 'C', 'D']) if (labeledBoxes[label] === cell(push.box)) labeledBoxes[label] = cell(push.boxTo);
...
export async function chooseWithPreview(brain, state, observation, { signal, stage, visits, onComparing }) {
  ...
  const proposal = await brain.json([
    { role: 'system', content: 'Play Sokoban. Return JSON {"stage":{"box":{"x":2,"y":3},"target":{"x":2,"y":2}},"replaceStage":false,"candidates":[{"pushes":[{"box":"A","direction":"U"},{"box":"A","direction":"R"}]}]}. boxLabels assigns A through D to the boxes in THIS observation. Labels follow the same box throughout each candidate, even after it moves; the engine updates coordinates. Keep activeStage until achieved; if revision is necessary explicitly set replaceStage=true. Choose a box/goal assignment leaving the other box a distinct goal. Propose up to THREE genuinely different branches of TWO to FOUR pushes; one push is enough if about to win or no useful continuation is known. First pushes must be available now; all continuations must preserve a reachable standing square behind the box. Walking is automatic with boxes fixed. You may move the other box to clear space. Do not fill a goal prematurely if it blocks another box. Never propose deadlock=true pushes. Use recentDecisions to avoid loops. No prose or full solution. To backtrack instead, return {"choice":"undo"} or {"choice":"restart"}.' },
    { role: 'user', content: JSON.stringify({ ...observation, activeStage, boxLabels: Object.fromEntries(state.boxes.map((b,i) => ['ABCD'[i], point(b)])) }) },
