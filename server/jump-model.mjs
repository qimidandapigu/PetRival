...
function actionSummary(actions) {
  return actions.map(a => `${a.move < 0 ? '左' : a.move > 0 ? '右' : '停'}${a.jump ? '跳' : ''}${a.frames}帧`).join('→');
}
function attemptsInput(raw, level) {
  const parsed = (Array.isArray(raw) ? raw.slice(-8) : []).map((entry, index) => ({
...
这个世界的精确物理（所有关卡相同，可直接用数值计算）：按住方向键每 1 帧水平移动 ${PHYSICS.speed} 像素，地上和空中都一样——「向右 N 帧」就是水平移动 ${PHYSICS.speed}×N 像素，比如向右 90 帧 = ${Math.round(PHYSICS.speed * 90)} 像素。起跳后水平速度只由按住的方向键决定，不按方向键就不会水平移动。算帧数时用这个换算，不要凭感觉猜。
