...
Source adds `/api/jump/lab/prior|induce|plan` and a browser-local blank-slate world where channel roles and physics are hidden per world. Both Node and the Worker implement the three lanes with separate per-minute limits; `lab/plan` shares the `play` lane and lease with `/api/jump/decision`. No schema migration and no identity, reward or existing-game change. 181 Node + 40 cloud tests, syntax scan and build passed; real-model checks ran against a local Node server. This update is **not published**: the `sites-app/` mirror and the deployed site still serve v17. See [behavior and evidence](JUMP-BLANK-PET-2026-09-15.md).
...
The jump page now renders two clipped panes over the same world by default (pet on top, the human below), each with its own camera, so both players stay visible when they separate; unchecking the box returns to the previous single-camera mode. A "restart round (keep learning)" action resets positions and the level objective while keeping the notebook, traces, world model and demonstrations. Client-only: no route, schema, identity or reward change, and both runtimes serve identical assets. 193 Node + 40 cloud tests, syntax scan and build passed. Not published; still v17. See [behavior and limits](JUMP-SPLIT-VIEW-2026-09-15.md).

## Independent players update

The human and the pet are now two full players on their own progress objects in `public/jump-world.mjs`: `step()` and `touch()` no longer special-case a role, so the human collects coins, carries the key, opens the door and can win on its own score, with the door blocking whoever has not opened it. Both sides respawn after a fall and reset only their own progress (the pet comes back by itself after 45 frames instead of stopping). The learning log and the "what it knows" panel move into a sticky left column on `/jump`. No route, schema, identity or reward change. The superseded `public/jump-engine.mjs` (not loaded by any page) keeps the old human-cannot-collect rule on purpose. 194 Node + 40 cloud tests, syntax scan and build passed. Not published; still v17. See [behavior, evidence and limits](JUMP-INDEPENDENT-PLAYERS-2026-09-15.md).

## Stage ladder update

`public/jump-stages.mjs` adds six verified lesson stages (walk, jump, coins, key, backtrack to the switch, then the original level), each proven solvable by the bounded solver: stage 1 is flat ground a pet can clear by walking right, stage 2 is a real 80px creek. The page shows a stage bar (cleared / current / locked), unlocks the next stage when **either** side clears the current one, and remembers the ladder position in browser storage. `/jump` becomes three columns — learning log (one element per line with a time and a colour-coded category chip), game, and a right-hand "what it knows" panel that works in both the lesson (progress, calls, falls, the model's current goal) and the lab (notebook and world model). Client-only: no route, schema, identity or reward change. 203 Node + 40 cloud tests, syntax scan and build passed. Not published; still v17. See [stage ladder and layout](JUMP-STAGE-LADDER-2026-09-15.md).
